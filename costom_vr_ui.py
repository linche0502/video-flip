...
# pyuic5 -x "D:\OneDrive\Documents\workspace\python\video-flip\video-flip\vr_ui.ui" -o "D:\OneDrive\Documents\workspace\python\video-flip\video-flip\vr_ui.py"
...
class MyMainWindow(Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def setOutputName(self):
        if filePath:= self.input_filePath.text():
            ...
            newfileName= os.path.basename(filePath).split('.')
            newfileName= ''.join(newfileName[:-1])+ '-'+ outputType+ '.'+ newfileName[-1]
            self.output_filePath.setText(os.path.join(os.path.dirname(filePath), newfileName))
    ...
    def run(self):
        ...
        try:
            ...
            # 分割左右眼的視訊, 0:左眼,1:右眼, 上下:左眼在上,右眼在下
            if iOptions[1] == "lr":
    ...
    def showMsg(self, msg:str, icon:str="information"):
        ...
        screen_index = QtWidgets.QDesktopWidget().screenNumber(main_window_rect.center())
        screen_geometry = QtWidgets.QDesktopWidget().screenGeometry(screen_index)
        # 建立訊息視窗, 並將msgBox移動到主視窗所在的螢幕的中間(not working)
        msgBox= QtWidgets.QMessageBox()
        msgBox.move(screen_geometry.center() - msgBox.rect().center())
        # 設定訊息類型和內容
        if icon == "information":
